...
void loadGame(char board[SIZE][SIZE]);
void editBoardWithLoad(string load, char board[SIZE][SIZE]);
...
void loadGame(char board[SIZE][SIZE]){
    ...
    string playerLine, computerLine;
    getline(inStream, playerLine);
    getline(inStream, computerLine);
    
    editBoardWithLoad(playerLine, board);
    editBoardWithLoad(computerLine, board);
}

void editBoardWithLoad(string load, char board[SIZE][SIZE]){
    char user = load.at(0);
    for(int i = 3; i < load.length(); i += 2){
        int play = load.at(i) - '0';
        board[(play-1)/3][(play-1)%3] = user;
    }
}
